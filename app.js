...
function renderTwitter(data){
  document.querySelector("#twitter-cards").innerHTML = data.items.slice(0,5).map(v=>`
    <a class="card tweet" href="${v.url}" target="_blank" rel="noopener">
      <div class="tweet-head"><span class="avatar"></span><b>${escapeHtml(v.author)}</b><span class="tweet-date">${v.date}</span></div>
      <div class="tweet-text">${escapeHtml(v.text)}</div>
      <div class="tweet-stats"><span>♡ ${formatNumber(v.likes)}</span><span>↗ ${formatNumber(v.reposts)}</span><span>◊ ${formatNumber(v.views)}</span></div>
    </a>`).join("");
}
